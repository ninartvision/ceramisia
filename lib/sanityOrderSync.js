...
export async function trySanityCreateOrder(client, doc, contextTag) {
  ...
  const rawCustomer = parseCustomerFromBody(doc);
  const validation = validateCustomer(rawCustomer);
  const customer = validation.ok
    ? validation.customer
    : buildSafeCustomerForSanity(rawCustomer);
  if (!validation.ok) {
    console.warn(`[Sanity:${tag}] invalid customer — creating fallback-safe order`, {
      orderId: orderId || null,
      errors: validation.errors,
    });
  }

  const payload = buildSanityOrderFields(customer, {
    ...doc,
    orderId,
    ...
    message: customer.message || doc.message || "",
  });
...
export async function syncSanityOrderOnPaymentSuccess(client, data, contextTag) {
  ...
  const customer = validation.ok
    ? validation.customer
    : buildSafeCustomerForSanity(mergedCustomer);
  if (!validation.ok) {
    console.warn(`[Sanity:${tag}] customer incomplete — syncing fallback-safe order`, {
      orderId,
      errors: validation.errors,
    });
  }

  const doc = buildSanityOrderFields(customer, {
    _id: docId,
    orderId,
    message:
