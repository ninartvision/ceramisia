...
export async function syncSanityOrderOnPaymentSuccess(client, data, contextTag) {
  ...
  // Customer-supplied (email-style) string can be valuable even when the bank
  // didn't echo a full name back — prefer the bank-provided name, then the
  // earlier checkout snapshot, then any callback-derived email/identifier,
  // then a generic placeholder so the required field validates in Studio.
  const customerName =
    (typeof data?.customerName === "string" && data.customerName.trim()) ||
    (typeof existing?.customerName === "string" &&
      existing.customerName.trim() &&
      existing.customerName.trim() !== "Unknown" &&
      existing.customerName.trim()) ||
